...
def impute_categorical_missing_data(passed_col):
    ...
    for cols in X.columns:
        if X[cols].dtype == 'object' or X[cols].dtype == 'category' :
    ...
    for cols in other_missing_cols:
        if X[cols].isnull().sum() > 0:
            cols_with_missing_value = X[cols].value.reshape(-1,1)
    ...
    for cols in other_missing_cols:
        if(X[cols].isnull().sum() > 0):
            cols_with_missing_values = X[cols].value.reshape(-1,1)
    ...
    if len(df_null) > 0: # < -> >
        df_null[passed_col] = rf_classifier.predict(X) ## classifier -> rf_classifier
        if passed_col in bool_cols:
...
#############################################################################################################
    ...
    if len(df_null) > 0:
        df_null[passed_col] = rf_regressor.predict(X_train) #rf_classifier -> rf_regressor
    else:
        pass
...
for i, cols in enumerate(numerical_cols):
    plt.subplot(3,2,i+1)
    sns.boxenplot(x=df[cols],color=palette[i % len(palette)])  # Use modulo to cycle through colors
    plt.title(cols)
...
for i, col in enumerate(numerical_cols): ## cols -> col
    plt.subplot(3,2,i+1)
...
for col in X.columns:
    if X[col].dtype == 'object' or X[cols].dtype == 'category' :
        X[col] = Label_Encoder.fit_transform(X[col].astype(str)) #onehotencoder -> Label_Encoder
    else:
        pass
...
def hyperparameter_tuning(X, y, categorical_columns, models):
    ...
    elif model_name == 'Naye base Classifier':
